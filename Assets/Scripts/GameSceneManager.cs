...
public class GameSceneManager : MonoBehaviour
{
    ...
    {
        yield return StartCoroutine(TransitionOut()); // 1. 變黑

        AsyncOperation op = SceneManager.LoadSceneAsync(sceneName); // 2. 載入
        while (!op.isDone) yield return null;

        yield return StartCoroutine(TransitionIn()); // 3. 變亮
    }
    ...
    private IEnumerator TransitionIn() // 變透明 (Loading 結束)
    {
        ...
        if (fadeCanvasGroup != null)
        {
            ...
            fadeCanvasGroup.blocksRaycasts = false; // 恢復滑鼠點擊
        }
    }
